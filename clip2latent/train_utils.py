...
@torch.no_grad()
def compute_val(diffusion, val_im, G, clip_model, device, stats, super_cond=0):
    diffusion.eval()
    images = []
    inp = val_im["clip_features"].to(device)
    out = diffusion.p_sample_loop(inp.shape, {"text_embed": inp}, clip_denoised=False, super_cond=super_cond)
