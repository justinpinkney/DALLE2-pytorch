...
class ZWPrior(DiffusionPrior):
    ...
    def p_sample(self, x, t, text_cond = None, clip_denoised = True, repeat_noise = False, super_cond=0):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x = x, t = t, text_cond = text_cond, clip_denoised = clip_denoised)
        if super_cond:
            text_cond = {k: 0*v for k, v in text_cond.items()}
            uncond_model_mean, _, uncond_model_log_variance = self.p_mean_variance(x = x, t = t, text_cond = text_cond, clip_denoised = clip_denoised)
